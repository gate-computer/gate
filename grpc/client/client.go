...
func New(ctx Context, conn *grpc.ClientConn) (*Conn, error) {
	...
	r, err := pb.NewRootClient(conn).Init(ctx, &pb.InitRequest{})
	if err != nil {
		return nil, err
	}
...
func newService(c pb.InstanceClient, info *pb.Service) *Service {
	return &Service{
		c:    c,
		info: info,
	}
...
func (s *Service) CreateInstance(ctx Context, config service.InstanceConfig, snapshot []byte) (service.Instance, error) {
	...
	r, err := s.c.Create(ctx, &pb.CreateRequest{
...
func newInstanceConfig(ctx Context, config service.InstanceConfig, key []byte) *pb.InstanceConfig {
	r := &pb.InstanceConfig{
...
type instance struct {
	...
	stream   pb.Instance_ReceiveClient
	leftout  <-chan []byte
	incoming []byte
}

func newInstance(ctx Context, c pb.InstanceClient, id []byte, config service.InstanceConfig) *instance {
	return &instance{
		c:    c,
		id:   id,
		code: config.Code,
...
func (inst *instance) Ready(ctx Context) error {
	stream, err := inst.c.Receive(ctx, &pb.ReceiveRequest{
		Id: inst.id,
	})
	if err != nil {
		return err
...
func (inst *instance) Handle(ctx Context, out chan<- packet.Thunk, p packet.Buf) (packet.Buf, error) {
	if len(inst.incoming) == 0 {
		_, err := inst.c.Handle(ctx, &pb.HandleRequest{
			Id:   inst.id,
			Data: p,
		})
		if err == nil {
...
func (inst *instance) Shutdown(ctx Context, suspend bool) ([]byte, error) {
	...
	if !suspend {
		_, err := inst.c.Shutdown(ctx, &pb.ShutdownRequest{
			Id: inst.id,
		})
		return nil, err
	}

	_, err := inst.c.Suspend(ctx, &pb.SuspendRequest{
		Id: inst.id,
	})
	if err != nil {
		return nil, err
	...
	r, err := inst.c.Snapshot(ctx, &pb.SnapshotRequest{
		Id:       inst.id,
		Outgoing: outgoing,
...
func receiveForward(ctx Context, code packet.Code, out chan<- packet.Thunk, stream pb.Instance_ReceiveClient, leftout chan<- []byte, abort func(error)) {
	defer close(leftout)
...
func receiveBuffer(initial packet.Buf, stream pb.Instance_ReceiveClient, abort func(error)) []byte {
	initial.SetSize()
	buf := initial
